...
public interface IOrderRepository extends JpaRepository<Order, Integer> {
    ...
    @Query("select obj from Order obj JOIN OrderDetail obj2 ON obj.id=obj2.order.id where obj2.items.id IS NULL and obj.status>0 GROUP BY obj.id")
    Page<Order> findAllOrderNick( Pageable pageable);

    @Query("select obj from Order obj JOIN OrderDetail obj2 ON obj.id=obj2.order.id where obj2.nickGame.id IS NULL and obj.status>0 GROUP BY obj.id")
