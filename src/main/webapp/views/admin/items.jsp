...
<div class="d-sm-flex align-items-center justify-content-between mb-4 offset-5">
        ...
        <tbody>
            <c:forEach items="${list.content}" var="items" varStatus="status">
                <tr>
                    ...
                    <td>
                        <a data-toggle="modal" data-target="#i${items.id}" class="btn btn-danger">Xóa</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
    ...
    <div class="row">
        ...
        <nav aria-label="Page navigation example" class="d-flex justify-content-end col-6">
            <ul class="pagination">
                <li ${list.number ==0 ? "hidden": ""} class="page-item"><a class="page-link" href="/admin/items/index?page=${list.number-1} ">Previous</a></li>
                <c:forEach var="i" begin="0" end="${ list.totalPages - 1 }">
                    <li class="page-item"><a class="page-link" href="/admin/items/index?page=${ i }">${ i + 1 }</a></li>
                    </li>
                </c:forEach>
                <li ${list.number ==list.totalPages-1 ? "hidden": ""} class="page-item"><a class="page-link" href="/admin/items/index?page=${list.number+1}">Next</a></li>
            </ul>
        </nav>
    </div>
<c:forEach items="${list.content}" var="items" varStatus="status">
